	...
	class Tabbed_Settings extends Extendible_Tabbed_Settings {
		...
		public function field_plugin_checkbox_option( array $args  ) {
			...
			if ( ! file_exists( $plugin_main_file ) ) {
				...
				if ( $value ) {
                                    echo '  <a href="' . add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, admin_url( 'themes.php' ) ) . '">' .  _x( 'Install', 'Install the Plugin', 'overwrite-author-name' ) . " </a> | " ;
                                }
				...
				if ( ! $value ) { 
                                    echo '<a href="plugins.php?s=' . esc_html( $option['label'] )	 . '">' .  _x( 'Deactivate', 'deactivate the plugin', 'overwrite-author-name' ) . "</a> | " ;	
                                }
			} else {
				echo esc_html__( 'Force Active', 'overwrite-author-name' ) . ' ( ';
			}
